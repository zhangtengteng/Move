...
import com.xuanit.move.util.ImageDownloaderUtil;
...
public class ImageDownloader {
	...
	public void imageDownload(String url, ImageView mImageView, String path,
			Context mActivity, OnImageDownload onImageDownload) {
		...
		String imageName = "";
		if (url != null) {
			imageName = ImageDownloaderUtil.getInstance().getImageName(url);
		}
		...
		// 文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。
			...
			if (mImageView != null) {
				// System.out.println("====执行MyAsyncTask --> "
				// + ImageDownloaderUtil.flag);
				ImageDownloaderUtil.flag++;
				task.execute();
				// 将对应的url对应的任务存起来
				map.put(url, task);
	 ...
	 * @return
	 */
	private Bitmap getBitmapFromFile(Context mActivity, String imageName,
			String path) {
		Bitmap bitmap = null;
		if (imageName != null) {
			File file = null;
	...
	private class MyAsyncTask extends AsyncTask<String, Void, Bitmap> {
		...
		public MyAsyncTask(String url, ImageView mImageView, String path,
				Context mActivity, OnImageDownload download) {
		...
		protected void onPostExecute(Bitmap result) {
			// 回调设置图片
			if (download != null) {
				download.onDownloadSuccess(result, url, mImageView);
	...
	public interface OnImageDownload {
		void onDownloadSuccess(Bitmap bitmap, String imgUrl, ImageView imageView);
	}
}
